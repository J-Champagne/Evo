...
public class InteractionInstanceService extends AbstractBCIInstanceService<InteractionInstance, BCIActivityClientEvent> {
    ...
    protected void checkAllEntryConditions(BCIActivityCheckEntryConditionsClientEvent event) {
        ...
        if (!event.getBciActivityId().equals(event.getNewBCIActivityId())) {
            InteractionInstance newActivityInstance = findById(event.getNewBCIActivityId());
    ...
    /**
     * Checks all the entry conditions of the InteractionInstance associated with the event.
     * If the entry conditions pass, the status of the activity is set to IN_PROGRESS and is updated in the database.
     * The response object is updated to reflect which entities were changed.
     *
     * @param event The associated event
     */
    @EventListener(BCIBlockInstanceToActivityCheckEntryConditionsClientEvent.class)
    protected void checkEntryConditionsAndUpdateWithResponse(BCIBlockInstanceToActivityCheckEntryConditionsClientEvent
                                                                         <InteractionInstance> event) {
        FailedConditions failedConditions = new FailedConditions();
        InteractionInstance interactionInstance = event.getActivityInstance();
        ...
        if (failedConditions.getFailedEntryConditions().isEmpty()) {
            ...
            event.getResponse().addResponse(
                    InteractionInstance.class.getSimpleName() + "_" + interactionInstance.getId(),
                    interactionInstance.getId(), interactionInstance.getStatus(), failedConditions.getFailedEntryConditions(),
                    failedConditions.getFailedExitConditions()
            );
        }
    }
